...
(function () {
    ...
    const Theme = {
        ...
        // White & green design tokens — the single source of truth for all colors.
        tokens: {
            panelGradient: "#ffffff",
            railGradient: "#ffffff",
            headerGradient: "#f0fdf4",
            buttonGradient: "linear-gradient(135deg, #2ecc71 0%, #27ae60 100%)",
            buttonGradientHover: "linear-gradient(135deg, #27ae60 0%, #1e8449 100%)",
            buttonSecondaryGradient: "#ffffff",
            railIconActive: "#2ecc71",
            textLight: "#0f4224",
            textMuted: "#27ae60",
            borderSoft: "#2ecc71",
            inputBg: "#ffffff",
            inputText: "#0f4224",
            shadow: "0 8px 28px rgba(39, 174, 96, 0.25)",
        },
        ...
        injectStyles() {
            ...
            style.textContent = `
                ...
                .ai-exporter-rail-btn:hover { background: #f0fdf4; color: ${t.textLight}; }
                ...
                .ai-exporter-icon-btn:hover { background: #f0fdf4; color: ${t.textLight}; }
                ...
                .ai-exporter-btn {
                    ...
                    padding: 0 10px; box-shadow: 0 2px 6px rgba(39,174,96,0.3);
                }
                ...
                .ai-exporter-btn.secondary:hover { background: #f0fdf4; color: ${t.textLight}; }
                .ai-exporter-count-pill {
                    font-size: 10px; font-weight: 700; background: #e9f7ef;
                ...
                #${Config.DOM.PANEL_ID} input[type="text"]:focus, #${Config.DOM.PANEL_ID} textarea:focus { outline: 2px solid #2ecc71; }
                ...
                .ai-exporter-outline-item:hover { background: #f0fdf4; }
                ...
                .ai-exporter-kbd {
                    background: #f0fdf4; color: ${t.textLight}; border: 1px solid ${t.borderSoft};
                ...
                #${Config.DOM.PANEL_ID} ::-webkit-scrollbar-thumb { background: rgba(39, 174, 96, 0.4); border-radius: 6px; }
                #${Config.DOM.PANEL_ID} ::-webkit-scrollbar-track { background: rgba(39, 174, 96, 0.05); }
                #${Config.DOM.PANEL_ID} { scrollbar-color: rgba(39, 174, 96, 0.4) rgba(39, 174, 96, 0.05); scrollbar-width: thin; }
            `;
    ...
    const Utils = {
        ...
        parseChatTitleAndTags(rawTitle) {
            ...
            cleanedTitle = cleanedTitle.replace(/(^|\s+)#\S+/g, " ").replace(/\s+/g, " ").trim();
    ...
    Lib.TurndownService = class TurndownService {
        constructor(options = {}) {
            this.rules = [];
            this.options = { headingStyle: "atx", hr: "___", bulletListMarker: "-", codeBlockStyle: "fenced", ...options };
        }
        addRule(key, rule) {
        ...
        turndown(rootNode) {
            ...
            const parsedRootNode = typeof rootNode === "string" ? new DOMParser().parseFromString(rootNode, "text/html").body : rootNode;
    ...
    Lib.jsonToToon = function jsonToToon(obj, indent = 0) {
        ...
        if (typeof obj === "string") {
            ...
            return /[\n,:"]/.test(escaped) || escaped === "" ? `"${escaped}"` : escaped;
        }
        if (Array.isArray(obj)) {
            if (obj.length === 0) return "[]";
            const allObjects = obj.every((item) => item && typeof item === "object" && !Array.isArray(item));
            if (allObjects && obj.length > 0) {
                const firstKeys = Object.keys(obj[0]);
                const uniform = obj.every((item) => {
                    const keys = Object.keys(item);
                    return keys.length === firstKeys.length && firstKeys.every((k) => keys.includes(k));
                });
                if (uniform) {
                    const rows = obj.map((item) =>
                        firstKeys.map((k) => (item[k] === null || item[k] === undefined ? "" : jsonToToon(item[k], 0))).join(",")
                    );
                    return rows.map((row) => `${nextSpaces}${row}`).join("\n");
                }
            }
            return obj.map((item) => `${nextSpaces}- ${jsonToToon(item, indent + 1)}`).join("\n");
        }
    ...
    const Snapcompact = {
        ...
        measure(ctx, fontSizePx) {
            ...
            return { w: ctx.measureText("0").width, h: Math.ceil(fontSizePx * this.LINE_HEIGHT_RATIO) };
        },
        ...
        wrap(text, cols) {
            ...
            for (const raw of text.split("\n")) {
                ...
                for (let i = 0; i < raw.length; i += cols) lines.push(raw.slice(i, i + cols));
            }
            return lines;
        },

        renderPages(text, { fontSizePx = 10, frameWidth = this.FRAME_WIDTH, maxFrameHeight = this.MAX_FRAME_HEIGHT } = {}) {
            ...
            for (let start = 0; start < allLines.length; start += rowsPerFrame) {
                ...
                pages.push({ canvas, chars: pageLines.join("\n").length, cols, rows: pageLines.length });
            }
            return pages;
        },
     ...
     *     newChatUrl    : string                      — used by act "open new chat"
     ...
     *   }
     *
     * ChatExporter and UI never branch on "if platform === X" — they
    ...
    Platforms.registry.chatgpt = {
        ...
        selectors: {
            article: "section[data-turn-role], section[data-testid^='conversation-turn-']",
            header: "h4",
            popupDiv: "popover",
            buttonSpecificClass: "text-sm",
        },
        titleFromDoc(doc) {
            return doc.title.replace(" - ChatGPT", "").trim() || Config.DEFAULT_CHAT_TITLE;
        },
        extractMessages(doc) {
            ...
            for (const article of articles) {
                const isUser =
                    ...
                    (article.querySelector(this.selectors.header)?.textContent?.trim() || "").toLowerCase().includes("you said");
                const contentHtml = article.querySelector(".markdown, .whitespace-pre-wrap") || article;
                const contentText = contentHtml.innerText.trim();
                if (!contentText) continue;
                messages.push(makeMessage(isUser ? "user" : "ai", chatIndex, contentHtml, contentText));
                if (!isUser) chatIndex++;
            }
            return messages;
        },
        registerTurndownRules(turndown) {
            ...
            turndown.addRule("chatgptRemoveReactions", {
                filter: (node) => node.nodeName === "DIV" && node.querySelector(':scope > div:nth-child(1) > button[data-testid="copy-turn-action-button"]'),
                replacement: () => "",
            });
            turndown.addRule("chatgptRemoveH6ChatGPTSaid", {
                filter: (node) => node.nodeName === "H6" && node.classList.contains("sr-only") && node.textContent.trim().toLowerCase().startsWith("chatgpt said"),
                replacement: () => "",
            });
            turndown.addRule("popup-div", {
                filter: (node) => node.nodeName === "DIV" && node.classList.contains(sel.popupDiv),
                replacement: (content) =>
                    "\n```\n" +
                    content
                        .replace(/<br\s*\/?>/gi, "\n")
            ...
            turndown.addRule("buttonWithSpecificClass", {
                filter: (node) => node.nodeName === "BUTTON" && node.classList.contains(sel.buttonSpecificClass),
                replacement: (content) => (content.trim() ? `__${content}__\n\n` : ""),
            });
            turndown.addRule("pre", {
                filter: "pre",
                replacement: (content, node) => {
                    let codeText = node.querySelector(".cm-content")?.innerText || node.querySelector("code")?.textContent || "";
                    let lang =
                        node.querySelector(".text-token-text-primary, .flex.items-center.text-token-text-secondary, .text-xs.font-sans")?.querySelector("span")?.textContent.trim() ||
                        node.querySelector(".text-token-text-primary, .flex.items-center.text-token-text-secondary, .text-xs.font-sans")?.textContent.replace(/Copy code|Run/gi, "").trim() ||
                        "";
                    if (!codeText && !node.querySelector("code")) return content;
                    if (!lang && node.querySelector("code")?.className) lang = (node.querySelector("code").className.match(/language-(\w+)/) || [])[1] || "";
    ...
    Platforms.registry.claude = {
        ...
        selectors: {
            message: ".font-claude-response:not(#markdown-artifact), [data-testid='user-message']",
            userMessage: '[data-testid="user-message"]',
            thinkingBlockClass: "transition-all",
            artifactBlockCell: ".artifact-block-cell",
        },
        titleFromDoc(doc) {
            return (doc.title || Config.DEFAULT_CHAT_TITLE).replace(/\s-\sClaude$/, "").trim();
        },
        extractMessages(doc) {
            ...
            items.forEach((item) => {
                ...
                if (!isUser) {
                    ...
                    Array.from(item.children).forEach((child) => {
                        const isThinking = child.className.includes(this.selectors.thinkingBlockClass);
                        const isArtifact = (child.className.includes("pt-3") && child.className.includes("pb-3")) || child.querySelector(this.selectors.artifactBlockCell);
        ...
        registerTurndownRules(turndown) {
            turndown.addRule("claudeCodeBlock", {
                filter: (node) => node.nodeName === "DIV" && node.querySelector(":scope > div:nth-child(2)") && node.querySelector(":scope > div:nth-child(3) > pre > code"),
                replacement: (content, node) =>
                    "\n\n```" +
                    (node.querySelector(":scope > div:nth-child(2)")?.textContent.trim().toLowerCase() || "") +
                    "\n" +
                    (node.querySelector(":scope > div:nth-child(3) > pre > code")?.textContent || "") +
                    "\n```\n\n",
            });
        },
    };
    ...
    Platforms.registry.copilot = {
        ...
        titleFromDoc(doc) {
            return (
                doc.querySelector('[role="option"][aria-selected="true"]')?.querySelector("p")?.textContent.trim() ||
                doc.querySelector('[role="option"][aria-selected="true"]')?.getAttribute("aria-label")?.split(",").slice(1).join(",").trim() ||
                (doc.title || "").replace(/^\s*Microsoft[_\s-]*Copilot.*$/i, "").replace(/\s*[-–|]\s*Copilot.*$/i, "").trim() ||
        ...
        extractMessages(doc) {
            ...
            for (const item of items) {
                const isUser = item.matches(this.selectors.userMessage);
                const contentElem = isUser ? item.querySelector('[data-contt="user-message"]') : item.querySelector(".group\\/ai-message-item");
                if (!contentElem) continue;
                messages.push(makeMessage(isUser ? "user" : "ai", chatIndex, isUser ? contentElem : contentElem.cloneNode(true), contentElem.innerText.trim()));
                if (!isUser) chatIndex++;
            }
            return messages;
        },
        registerTurndownRules(turndown) {
            ...
            turndown.addRule("copilotCodeBlock", {
                filter: (node) => node.nodeName === "DIV" && node.querySelector(":scope > div:nth-child(1) span") && node.querySelector(":scope > div:nth-child(2) > div > pre"),
                replacement: (content, node) =>
                    "\n\n```" +
                    (node.querySelector(":scope > div:nth-child(1) span")?.textContent.trim().toLowerCase() || "") +
                    "\n" +
                    (node.querySelector(":scope > div:nth-child(2) > div > pre > code")?.textContent || "") +
                    "\n```\n\n",
            });
            turndown.addRule("copilotFooterLinks", {
                filter: (node) => node.nodeName === "A" && node.querySelector(":scope > span:nth-child(1)") && node.querySelector(":scope > img:nth-child(2)") && node.querySelector(":scope > span:nth-child(3)"),
                replacement: (content, node) => `[${node.querySelector(":scope > span:nth-child(3)")?.textContent.trim() || node.getAttribute("href")}](${node.getAttribute("href")}) `,
            });
        },
    };
    ...
    Platforms.registry.gemini = {
        ...
        selectors: {
            ...
            sidebarActiveChat: 'a[data-test-id="conversation"].selected .conversation-title',
            topbarActiveChat: "conversation-actions .conversation-title",
        },
        titleFromDoc(doc) {
            let title = doc.querySelector(this.selectors.sidebarActiveChat)?.textContent.trim() || doc.querySelector(this.selectors.topbarActiveChat)?.textContent.trim() || doc.title;
            if (title.startsWith("Gemini - ")) title = title.replace("Gemini - ", "").trim();
            const prefix = Store.get(Config.GM_KEYS.CHAT_TITLE_PREFIX, Config.DEFAULT_CHAT_TITLE_PREFIX);
            if (prefix && title.startsWith(prefix.trim())) {
                title = title.replace(new RegExp(`^${prefix.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*`), "").trim();
            }
            return title;
        },
        extractMessages(doc) {
            ...
            for (const item of items) {
                ...
                if (!contentElem) continue;
                messages.push(
                    makeMessage(isUser ? "user" : "ai", chatIndex, contentElem, contentElem.innerText.replace(/^you said\s+/i, "").trim())
                );
                if (!isUser) chatIndex++;
            }
            return messages;
        ...
        registerTurndownRules(turndown) {
            turndown.addRule("geminiCodeLanguageLabel", {
                filter: (node) => node.nodeName === "SPAN" && node.closest(".code-block-decoration") && node.textContent.trim().length > 0,
                replacement: () => "",
            });
            turndown.addRule("pre", {
                filter: "pre",
                replacement: (content, node) => {
                    const codeText = node.querySelector(".cm-content")?.innerText || node.querySelector("code")?.textContent || "";
                    const lang = node.closest(".code-block")?.querySelector(".code-block-decoration span")?.textContent.trim() || "";
                    if (!codeText && !node.querySelector("code")) return content;
                    return `\n\`\`\`${lang.toLowerCase()}\n${codeText.trim()}\n\`\`\`\n`;
                },
            });
    ...
    Platforms.registry.grok = {
        ...
        titleFromDoc(doc) {
            return (doc.title || Config.DEFAULT_CHAT_TITLE).replace(/\s-\sGrok$/, "").trim();
        },
        extractMessages(doc) {
            const items = [...doc.querySelectorAll(this.selectors.message)];
            const messages = [];
            ...
            for (const item of items) {
                ...
                if (!contentElem) continue;
                const textSource = isUser ? contentElem : item.querySelector(".response-content-markdown") || contentElem;
                messages.push(makeMessage(isUser ? "user" : "ai", chatIndex, isUser ? contentElem : contentElem.cloneNode(true), textSource.innerText.trim()));
                if (!isUser) chatIndex++;
            }
            return messages;
        },
        registerTurndownRules(turndown) {
            turndown.addRule("grokPreserveNewlines", {
                filter: (node) => node.nodeName === "P" && node.getAttribute("style")?.includes("white-space: pre-wrap"),
                replacement: (content) => `\n\n${content.trim().replace(/\n/g, "\n\n")}\n\n`,
            });
            turndown.addRule("grokCodeBlock", {
                filter: (node) => node.nodeName === "DIV" && node.getAttribute("data-testid") === "code-block",
                replacement: (content, node) => {
                    const rawLang = node.querySelector(".font-mono.text-xs")?.textContent.trim().toLowerCase() || "";
                    ...
                    if (node.querySelector(".font-mono.text-xs")) {
                        const lbl = node.querySelector(".font-mono.text-xs").textContent.trim();
    ...
    Platforms.detectCurrent = function detectCurrent() {
        ...
        return Object.values(Platforms.registry).find((p) => p.hostnames.some((h) => hostname.includes(h))) || null;
    };
    ...
    const ChatExporter = {
        ...
        buildTurndown(platform) {
            ...
            if (typeof platform.registerTurndownRules === "function") platform.registerTurndownRules(turndown);
            return turndown;
        },
        ...
        formatters: {
            markdown(chatData, turndown) {
                ...
                return {
                    output: yaml + `\n# ${chatData.title}\n\n## Table of Contents\n\n${toc.trim()}\n\n` + content.trim() + "\n\n",
                    fileName: Utils.formatFileName(Store.get(Config.GM_KEYS.OUTPUT_FILE_FORMAT, Config.DEFAULT_OUTPUT_FILE_FORMAT), chatData.title, chatData.tags, "md", chatData.platformId),
            ...
            json(chatData, turndown) {
                const jsonOutput = {
                    ...
                    messages: chatData.messages.map((msg) => {
                        ...
                        return { id: msg.id.split("-").slice(0, 2).join("-"), author: msg.author, content };
                    }),
                };
                return {
                    output: JSON.stringify(jsonOutput, null, 2),
                    fileName: Utils.formatFileName(Store.get(Config.GM_KEYS.OUTPUT_FILE_FORMAT, Config.DEFAULT_OUTPUT_FILE_FORMAT), chatData.title, chatData.tags, "json", chatData.platformId),
        ...
        initiateExport(format, platform) {
            ...
            if (!rawChatData || rawChatData.messages.length === 0) return alert("No messages found to export.");

            ChatExporter._selectedMessageIds.clear();
            const outlineListEl = document.querySelector(`#${Config.DOM.OUTLINE_LIST_ID}`);
            if (outlineListEl) {
                const visibleUserMessageIds = new Set();
                outlineListEl.querySelectorAll(".outline-item-checkbox:checked").forEach((cb) => {
                    const parent = cb.closest("div");
                    if (parent && window.getComputedStyle(parent).display !== "none" && cb.dataset.messageId) {
                ...
                rawChatData.messages.forEach((msg, index) => {
                    ...
                    for (let i = index - 1; i >= 0; i--) {
                        if (rawChatData.messages[i].author === "user") {
                            if (visibleUserMessageIds.has(rawChatData.messages[i].id)) ChatExporter._selectedMessageIds.add(msg.id);
                            break;
                        }
                    }
                });
            ...
            const filteredMessages = rawChatData.messages.filter((msg) => ChatExporter._selectedMessageIds.has(msg.id));
            if (filteredMessages.length === 0) return alert("No messages selected or visible for export.");

            const exportData = {
                ...rawChatData,
                tags: (rawChatData.tags || []).map((t) => t.toLowerCase().replace(/\s+/g, "").replace(/[^a-z0-9_-]/g, "")).filter((t) => t.length > 0),
            ...
            if (res.output && res.fileName) Utils.downloadFile(res.fileName, res.output, res.mimeType);
        },
    };
        ...
        registerAll(turndown, platform) {
            ...
            turndown.addRule("customLi", {
                ...
                replacement: (content, node) => {
                    ...
                    if (processed.length > 0 && processed.split("\n").length > 1 && /^\s*[-*+]|^[0-9]+\./.test(processed.split("\n")[1])) {
                    ...
                    if (node.parentNode.nodeName === "OL") {
                        return `${Array.from(node.parentNode.children).filter((c) => c.nodeName === "LI").indexOf(node) + 1}. ${processed}\n`;
                    }
                    return processed + "\n";
                },
            });
            turndown.addRule("code", {
                filter: "code",
                replacement: (content, node) =>
                    node.parentNode.nodeName === "PRE" || (platform.id === "grok" && node.closest && node.closest('[data-testid="code-block"]')) ? content : `\`${content}\``,
            });
            turndown.addRule("strong", { filter: ["strong", "b"], replacement: (content) => `**${content}**` });
            turndown.addRule("em", { filter: ["em", "i"], replacement: (content) => `_${content}_` });
            turndown.addRule("blockQuote", {
                filter: "blockquote",
                replacement: (content) => content.trim().split("\n").map((l) => `> ${l}`).join("\n"),
            });
            turndown.addRule("link", { filter: "a", replacement: (content, node) => `[${content}](${node.getAttribute("href")})` });
            turndown.addRule("strikethrough", { filter: (node) => node.nodeName === "DEL", replacement: (content) => `~~${content}~~` });
            turndown.addRule("table", {
                filter: "table",
                replacement: (content, node) => {
                    const rowText = (r) => Array.from(r.querySelectorAll("th, td")).map((c) => c.textContent.replace(/\s+/g, " ").trim());
                    ...
                    paddedRows.forEach((r, i) => {
                        md += "| " + r.join(" | ") + " |\n";
                        if (i === 0) md += "|" + Array(maxCols).fill("---").join("|") + "|\n";
                    });
                    return md.trim();
                },
            });
            turndown.addRule("paragraph", {
                ...
                replacement: (content, node) => {
                    ...
                    let p = node.parentNode;
                    while (p) {
                        if (Config.PARAGRAPH_FILTER_PARENT_NODES.includes(p.nodeName)) return content;
            ...
            turndown.addRule("images", {
                ...
                replacement: (content, node) => (node.getAttribute("src") ? `![${node.alt || ""}](${node.getAttribute("src")})` : ""),
            });
            turndown.addRule("sub", { filter: ["sub"], replacement: (content) => (content.trim() ? `<sub>${content}</sub>` : "") });
            turndown.addRule("sup", { filter: ["sup"], replacement: (content) => (content.trim() ? `<sup>${content}</sup>` : "") });
    ...
    const UI = {
        ...
        ensurePanel() {
            ...
            rail.appendChild(UI.makeRailButton({ id: "export-md label: "MD", tooltip: `AI Utils v${Config.VERSION}: Export to Markdown (ALT+M)`, onClick: () => UI.showSection("export-md") }));
            rail.appendChild(UI.makeRailButton({ id: "export-json", label: "JD", tooltip: `AI Utils v${Config.VERSION}: Export to JSON (ALT+J)`, onClick: () => UI.showSection("export-json") }));
            rail.appendChild(UI.makeRailButton({ id: "settings", label: "URL", tooltip: "Configure output filename format", onClick: () => UI.showSection("settings") }));
            rail.appendChild(UI.makeRailButton({ id: "toon", label: "JT", tooltip: "JSON → TOON converter", onClick: () => UI.showSection("toon") }));
            rail.appendChild(UI.makeRailButton({ id: "minifier", label: "TM", tooltip: "Text Minifier", onClick: () => UI.showSection("minifier") }));
            rail.appendChild(UI.makeRailButton({ id: "snapcompact", label: "IC", tooltip: "Snapcompact — Render compact image", onClick: () => UI.showSection("snapcompact") }));

            if (UI._platform?.hasAutoScroll                const scrollBtn = UI.makeRailButton({ id: "auto-scroll", emoji: "⬆️", label: UI.autoScrollEnabled ? "ON" : "OFF", tooltip: `Auto-scroll (ALT+A)`, onClick: () => UI.toggleAutoScroll() });
            ...
            const eyeBtn = UI.mailButton({ id: "master-eye", emoji: "👁️", label: "", tooltip: "Show / Hide AI Exporter Panel", onClick: () => UI.toggleGlobalCollapse() });
            ...
            conteader.id = "ai-exporter-content-header";
            ...
            contentHeader.appendChild(cseBtn);
            ...
            window.addEventListener("resize", () => UI.clampPanelToViewport(document.getElementById(Config.DOM.PANEL_ID)));
            return panel;
        },
        ...
        clampPanelToViewport(panel) {
            ...
            if (left > maxX) { left = Math.max(4, maxX); changed = true; }
            if (top > maxY) { top = Math.max(4, maxY); changed = true; }
            if (left < 4) { left = 4; changed = true; }
            if (top < 4) { top = 4; changed = true; }
            if (changed || panel.style.left) { panel.style.left = `${left}px`; panel.style.right = "auto"; }
            if (changed || panel.style.top) { panel.style.top = `${top}px`; panel.style.bottom = "auto"; }
            UI.checkScreenEdgesForContent();
        },
        ...
        toggleGlobalCollapse() {
            ...
            const panel = document.getElementById(Config.DOM.PANEL_ID);
            if (panel) panel.classList.toggle("global-collapsed", UI._globalCollapsed);
        ...
        toggleAutoScroll() {
            ...
            if (tip) tip.textContent = `Auto-scroll is ${isEnabled ? "ON" : "OFF"} (ALT+A)`;
        },
        ...
        showSection(id) {
            ...
            document.querySelectorAll(".ai-exporter-rail-btn").forEach((b) => b.classList.toggle("active", b.dataset.sectionId === id));
            ...
            while (contentBody.firstChild) contentBody.removeChild(contentBody.firstChild);
            document.getElementById("ai-exporter-content-title").textContent = UI._sections[id].title;
            contentBody.appendChild(UI._sections[id].body);
            contentEl.classList.add("open");
            if (typeof UI._sections[id].onShow === "function") UI._sections[id].onShow();
        ...
        closeSection() {
            ...
            document.querySelectorAll(".ai-exporter-rail-btn").forEach((b) => b.classList.remove("active"));
            const contentEl = document.getElementById("ai-exporter-content");
        ...
        sections: {
            ...
            buildOutlineBody(downloadLabel, downloadFormat) {
                ...
                downloadBtn.onclick = () => ChatExporter.initiateExport(downloadFormat, UI._platform);
                ...
                noMatchMessage.style.textign = "center";
                ...
                const updateSelectedCountDisplay = () => {
                    let selected = 0;
                    messageListDiv.querySelectorAll(".outline-item-checkbox:checked").forEach((cb) => {
                        if (cb.closest("div")?.style.display !== "none") selected++;
                    });
                    document.querySelectorAll('[id^="ai-exporter-outline-count-"]').forEach((el) => (el.textContent = `${selected} / ${userQuestionCount}`));
                };
                ...
                const renderItems = () => {
                    ...
                    ChatExporter._currentChatData.messages.forEach((msg) => {
                        ...
                        checkbox.onchange = () => {
                            masterCheckbox.checked = Array.from(messageListDiv.querySelectorAll(".outline-item-checkbox:not([style*='display: none'])")).every((cb) => cb.checked);
                        ...
                        itemText.onclick = () => ChatExporter._currentChatData.messages.find((m) => m.id === msg.id)?.contentHtml?.scrollIntoView({ behavior: "smooth", block: "start" });
                ...
                masterCheckbox.onchange = (e) => {
                    messageListDiv.querySelectorAll(".outline-item-checkbox:not([style*='display: none'])").forEach((cb) => (cb.checked = e.target.checked));
                    updateSelectedCountDisplay();
                };
                searchInput.oninput = () => {
                    ...
                    if (searchText) {
                        try {
                            ...
                            noMatchMessage.style.color = "#27ae60";
                            noMatchMessage.style.display = "block";
                            messageListDiv.style.display = "none";
                            outlineItemElements.forEach((item) => (item.style.display = "none"));
                    ...
                    for (let i = 0; i < messages.length; i++) {
                        if (messages[i].author === "user") {
                            map.set(messages[i].id, { user: messages[i], ai: i + 1 < messages.length && messages[i + 1].author === "ai" ? messages[i + 1] : null });
                        }
                    }
                    outlineItemElements.forEach((item, id) => {
                        const pair = map.get(id);
                        let match = false;
                        if (pair) match = searchText === "" || (searchRegex && (searchRegex.test(pair.user.contentText) || searchRegex.test(pair.ai ? pair.ai.contentText : "")));
                    ...
                    const visible = messageListDiv.querySelectorAll(".outline-item-checkbox:not([style*='display: none'])");
                    masterCheckbox.checked = visible.length > 0 && Array.from(visible).every((cb) => cb.checked);
                    updateSelectedCountDisplay();
                };
            ...
            registerExportSections() {
                const md = UI.sections.buildOutlineBody("↓ Download Markdown", "markdown");
                UI._sections["export-md"] = { title: "↓ Markdown export", body: md.wrap, onShow: md.renderItems };
                UI._renderOutlineItemsMd = enderItems;
                const json = UI.sections.buildOutlineBody("↓ Download JSON", "json");
                UI._sections["export-json"] = { title: "↓ JSON export", body: json.wrap, onShow: json.renderItems };
            ...
            registerSettingsSection() {
                ...
                UI._sections["settings"] = { title: "⚙️ Settings", body: wrap };
            },
            ...
            registerMinifierSon() {
                ...
                textInput.placeholder = "Enter text with extra spaces, tabs, line breaks...";
                ...
                minifyBtn.onclick = () => (textOutput.value = textInput.value.replace(/\s+/g, " ").trim());
                copyTextBtn.onclick = () =>
                    navigator.clipboard.writeText(textOutput.value).then(() => {
                        copyTextBtn.textContent = "Copied!";
                        setTimeout(() => (copyTextBtn.textContent = "Copy Result"), 1500);
                    });
                UI._sections["minifier"] = { title: "🧹 Text Minifie", body: wrap };
            },
            ...
            registerToonSection() {
                ...
                fileInput.onchange = (e) => {
                    ...
                    reader.onload = (event) => {
                        try {
                            jsonInput.value = JSON.stringify(JSON.parse(event.target.result), null, 2);
                        } catch {
            ...
            registerSnapactSection() {
                ...
                wrap.appendChsourceSelect);
                const customInput = document.createElement("textarea");
                customInput.style.height = "60px";
                customInput.style.display = "none";
                customInput.placeholder = "Paste text here...";
                wrap.appendChild(customInput);
                sourceSelect.onchange = () => (customInput.style.display = sourceSelect.value === "custom" ? "block" : "none");
                ...
                fontInput.max = "24";
                fontInput.value = "10";
                fontInput.style.width = "90px";
                wrap.appendChild(fontInput);
                const renderB = document.createElement("button");
                renderBtn.className = "ai-exporter-btn";
                renderBtn.textContent = "🖼️ Preview Render";
                ...
                results.stoverflowY = "auto";
                wrap.appendChild(results);

                function getSelectedMessagesForSnapcompact(chatData) {
                    const selectedIds = new Set();
                    document.getElementById(Config.DOM.OUTLINE_LIST_ID)?.querySelectorAll(".outline-item-checkbox:checked").forEach((cb) => {
                    ...
                    return chatData.messages.filter((m, idx) => {
                        ...
                        for (let i = idx - 1; i >= 0; i--) if (chatData.messages[i].author === "user") return selectedIds.has(chatData.messages[i].id);
                        return false;
                    });
                }
                ...
                const getPages = () => {
                    ...
                    if (sourceSelect.value === "custom") {
                        ...
                        try {
                            ...
                            if ((fChar === "{" && lChar === "}") || (fChar === "[" && lChar === "]")) text = Lib.jsonToToon(JSON.parse(text));
                        } catch (e) {
                            /* not JSON — fall through to plain text */
                        }
                        text = text.replace(/\s+/g, " ").trim();
                    else {
                        const msgs = sourceSelect.value === "chat-selected" ? getSelectedMessagesForSnapcompact(ChatExporter._currentChatData) : ChatExporter._currentChatData.messages;
                        text = Snapcompact.serialize(msgs);
                    }
                    return text ? Snapcompact.renderPages(text, { fontSizePx: parseInt(fontInput.value, 10) || 10 }) : [];
                };

                renderBtn.onclick = () => {
                    if (!ChatExporter._currentChatData && sourceSelect.value !== "custom") return alert("No chat data found.");
                    const pages = getPages();
                    if (!pages.length) return alert("Nothing to render — selection is empty.");

                    while (results.firstChild) results.removeChild(results.firstChild);
                    status.textContent = `${pages.reduce((sum, p) => sum + p.chars, 0).toLocaleString()} chars → ${pages.length} image(s)`;

                    // Only offer "Copy Image & Open New C when everything fit on one page.
                    if (pages.length === 1) {
                        ...
                        newChatBtn.onclick = async () => {
                            ...
                            pages[0].canvas.toBlob(async (blob) => {
                                try {
                                    ...
                                    setTimeout(() => (newChatBtn.textContent = "Copy Image & Open New Chat"), 3000);
                                    alert("✅ Image copi successfully!\n\nClick OK to open a new chat window. Once there, just press Ctrl+V (or Cmd+V) to paste.");
                    ...
                    pages.forEach((page, i) => {
                        ...
                        previewImg.src = page.caas.toDataURL("image/png");
                                ...
                                (b) =>
                                    Utils.downloadBlob(
                                        Utils.formatFileName(
                                            Store.get(Config.GM_KEYS.OUTPUT_FILE_FORMAT, Config.DEFAULT_OUTPUT_FILE_FORMAT),
                ...
                UI._sections["snapcompact"] = { title: "🖼️ Snapcompact"dy: wrap };
            },
        },
        ...
        refresh() {
            ...
            if (Object.keys(UI._sections).length === 0) {
                UI.buildAllSections();
                if (UI._activeSectionId && UI._sections[UI._activeSectionId] && !UI._globalCollapsed) UI.showSection(UI._activeSectionId);
            }
            if (!UI._platform) {
                panel.style.display = "none";
                return;
            }
            const freshChatData = ChatExporter.extractChatData(UI._platform, document);
            ...
            const hasDataChanged =
                ...
                (freshChatData.messages.length > 0 &&
                    ...
                    freshChatData.messages[freshChatData.messages.length - 1].contentText !== prev.messages[prev.messages.length - 1].contentText);

            if (!hasDataChanged) {
                panel.style.display = freshChatData && freshChatData.messages.length > 0 ? "flex" : "none";
                return;
            }
            ...
            if (typeof UI._renderOutlineItemsMd === "function") UI._renderOutlineItemsMd();
            if (typeof UI._renderOutlineItemsJson === "function") UI._renderOutlineItemsJson();
        },

        /** Gemini-only: repeatedly scrolls to top to force-load full chat history before extraction. */
        async autoScrollToTop() {
            if (UI.autoScrollEnabled === false || !UI._platform?.hasAutoScroll) return;
            const currentUrl = Utils.getCleanUrl();
            if (UI._lastProcessedChatUrl === currentUrl) return;
            const scrollableElement =
                document.querySelector('[data-test-id="chat-history-container"]') || document.querySelector("#chat-history") || document.querySelector("main") || document.documentElement;
            ...
            const waitForElementToAppear = (selector, timeoutMs, checkInterval = POLL_INTERVAL) => {
                const startTime = Date.now();
                return new Promise((resolve) => {
                    const interval = setInterval(() => {
                        const element = document.querySelector(selector);
                        if (element) { clearInterval(interval); resolve(element); }
                        else if (Date.now() - startTime > timeoutMs) { clearInterval(interval); resolve(null); }
                    }, checkInterval);
                });
            };
            const waitForElementToDisappear = (selector, timeoutMs, checkInterval = POLL_INTERVAL) => {
                const startTime = Date.now();
                return new Promise((resolve) => {
                    const interval = setInterval(() => {
                        const element = document.querySelector(selector);
                        if (!element || (element.offsetWidth === 0 && element.offsetHeight === 0)) { clearInterval(interval); resolve(true); }
                        else if (Date.now() - startTime > timeoutMs) { clearInterval(interval); resolve(false); }
                    }, checkInterval);
                });
            };

            const initialMessageElement = await waitForElementToAppear(UI._platform.selectors.messageItem, MESSAGE_APPEAR_TIMEOUT);
            if (!initialMessageElement) { UI._lastProcessedChatUrl = null; return; }
            UI._lastProcessedChatUrl = currentUrl;
            if (!UI._initialListenersAttached) { UI.initUrlChangeObserver(); UI._initialListenersAttached = true; }

            while (true) {
                scrollableElement.scrollTop = 0;
                await delay(50);
                const progressBarElement = await waitForElementToAppear("mat-progress-bar.mdc-linear-progress--indeterminate", PROGRESS_APPEAR_TIMEOUT);
                if (progressBarElement) {
                    retriesForProgressBar = 0;
                    await waitForElementToDisappear("mat-progress-bar.mdc-linear-progress--indeterminate", PROGRESS_DISAPPEAR_TIMEOUT);
                } else {
                ...
                const currentChatData = ChatExporter.extractChatData(UI._platform, document);
        ...
        handleUrlChange() {
            ...
            if (UI._platform?.hasAutoScroll && UI._platform.hostnames.some((host) => newUrl.includes(host)) && newUrl.includes("/app")) {
                ...
                window.addEventListener(
                    ...
                    () => {
                        ...
                        scrollTimeout = setTimeout(() => {
                            const newChatData = ChatExporter.extractChatData(UI._platform, document);
                            const prev = ChatExporter._currentChatData;
                            if (newChatData && prev && (newChatData._raw_title !== prev._raw_title || newChatData.messages.length > prev.messages.length)) UI.refresh();
                        }, 500);
                    },
                    true
                );
        ...
        initUrlChangeObserver() {
            ...
            (function (history) {
                ...
                history.pushState = function (state) {
                    if (typeof history.onpushstate === "function") history.onpushstate({ state });
                    window.dispatchEvent(new Event("customHistoryChange"));
                    return pushState.apply(history, arguments);
                };
                const replaceState = history.replaceState;
                history.replaceState = function (state) {
                    if (typeof history.onreplacestate === "function") history.onreplacestate({ state });
                    window.dispatchEvent(new Event("customHistoryChange"));
        ...
        setupShortcuts() {
            document.addEventListener("keydown", (e) => {
                ...
                if (e.altKey && e.code === "KeyM") { e.preventDefault(); ChatExporter.initiateExport("markdown", UI._platform); }
                if (e.altKey && e.code === "KeyJ") { e.preventDefault(); ChatExporter.initiateExport("json", UI._platform); }
                if (UI._platform?.hasAutoScroll && e.altKey && e.code === "KeyA") { e.preventDefault(); UI.toggleAutoScroll(); }
            });
        },
        ...
        init() {
            ...
            const start = () => {
                setTimeout(() => {
                    ...
                    UI.buildAllSections();
                    UI.refresh();
                    if (UI._platform.hasAuScroll) setTimeout(() => UI.autoScrollToTop(), Config.AUTOSCROLL_INITIAL_DELAY_MS);
    ...
    GM_registerMenuCommand("Set Gemini Chat Title Prefix", () => {
        const currentPrefix = Store.get(Config.GM_KEYS.CHAT_TITLE_PREFIX, Config.DEFAULT_CHAT_TITLE_PREFIX);
