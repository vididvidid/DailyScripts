/*
 * File: DownloadChatandMinifyJSONandText.js
 * Author: vididvidid 
 * Created: 2026-06-03 13:41:26
 */
...
// @version      3.2.0
// @description  Export your ChatGPT, Claude, Copilot, Gemini or Grok chat into a properly and elegantly formatted Markdown or JSON. Includes Text Minifier & JSON→TOON converter.
...
(function () {
      ...
      if (CURRENT_PLATFORM === GROK) {
        ...
        turndownServiceInstance.addRule("grokCodeBlock", {
          ...
          replacement: function (content, node) {
            ...
            // a fenced block inside content — extract it and re-wrap with correclang.
  ...
  const UIManager = {
    ...
    addToolsButton() {
      if (document.querySelector("#exporter-tools-btn")) return;
      const container = document.querySelector(`#${EXPORT_CONTAINER_ID}`);
      if (!container) return;

      const toolsBtn = document.createElement("button");
      toolsBtn.id = "exporter-tools-btn";
      toolsBtn.textContent = "🛠️ Tools";
      toolsBtn.title = "Text Minifier & JSON → TOON converter";
      Utils.applyStyles(toolsBtn, {
        ...BUTTON_BASE_PROPS,
        marginLeft: "8px",
      });
      toolsBtn.onclick = () => UIManager.toggleToolsModal();
      container.appendChild(toolsBtn);
    },
      /**
     le the tools modal visibility.
     */
    toggleToolsModal() {
      let modal = document.querySelector("#exporter-tools-modal");
      if (!modal) {
        modal = UIManager.buildToolsModal();
    ...
    /**
     * Build and return the tools modal DOM element.
     */
    buildToolsModal() {
      const modal = document.createElement("div");
      modal.id = "exporter-tools-modal";
      ...
      overlay.id = "exporter-tools-overlay";
      Utils.applyStyles(overlay, {
        position: "fixed",
        top: 0,
        left: 0,
        width: "100vw",
...
headerSpan.textContent = "🛠️ Export Tools";
header.appendChild(headerSpan);

      const closeBtn = document.createElement("button");
      closeBtn.textContent = "✕";
      Utils.applyStyles(closeBtn, {
        background: "none",
        border: "
        fontSize: "20px",
        cursor: "pointer",
      ...
      textTab.textContent = "Text Minifier";
      const jsonTab = document.createElement("button");
      jsonTab.textContent = "JSON → TOON";

      [textTab, jsonTab].forEach((tab) => {
        Utils.applyStyles(tab, {
          flex: 1,
          padding: "10px 0",
          background: "none",
          border: "none",
          ...
          fontWeight: "600",
          color: "#555",
          borderBottom: "3pxolid transparent",
          transition: "all 0.2s",
        });
      });
      ...
      [textContent, jsonContent].forEach((div) => {
        Utils.applyStyles(div, {
          padding: "16px",
        });
      });
      ...
      textInputLabel.textContent = "Paste text:";
      Utils.applyStyles(textInputLabel, { display: "block", marginBottom: "4px", fontWeight: "600" });
      textContent.appendChild(textInputLabel);

      const textInput = document.createElement("textarea");
      textInput.placeholder = "Enter text with extra spaces, tabs, line breaks...";
      Utils.applyStyles(textInput, {
        width: "100%",
        height: "100px",
        padding: "8px",
      ...
      minifyBtn.textContent = "Minify Text";
      Utils.applyStyles(minifyBtn, {
        marginTop: "8px",
        padding: "8px 16px",
        backgroundColor: "#5b3f87",
        color: "#fff",
      ...
      copyTextBtn.textContent = "Copy Minified";
      Utils.applyStyles(copyTextBtn, {
        marginTop: "4px",
        padding: "6px 12px",
        backgroundColor: "#eee",
        border: "1px solid #ccc",
      ...
      minifyBtn.onclick = () => {
        ...
        // Minify: collapse all whitespace sequences (including newlines) into a single space, trim
        const minified = original.replace(/\s+/g, " ").trim();
        textOutput.value = minified;
      };

      copyTextBtn.onclick = () => {
        navigator.clipboard.writeText(textOutput.value).then(() => {
          copyTextBtn.textContent = "Copied!";
          setTimeout(() => (copyTextBtn.textContent = "Copy Minified"), 1500);
        });
      };

      textContent.appendChild(minifyBtn);
      textContent.appendChild(textOutput);
      textContent.appendChild(copyTextBtn);

      // ---- JSON to TOON Tab ----
      const jsonInputLabel = document.createElement("label");
      jsonInputLabel.textContent = "Paste JSON:";
      jsonInputLabel.style.display = "block";
      jsonInputLabel.style.marginBottom = "4px";
      jsonInputLabel.style.fontWeight = "600";
      jsonContent.appendChild(jsonInputLabel);

      const jsonInput = document.createElement("textarea");
      jsonInput.placeholder = '{"key": "value", ...}';
      Utils.applyStyles(jsonInput, {
        width: "100%",
        height: "100px",
        padding: "8px",
        border: "1px solid #ccc",
      ...
      fileInput.onchange = (e) => {
        ...
        reader.onload = (event) => {
          ...
          // Try to prettify JSON if it's valid, otherwise show raw text
          try {
            const parsed = JSON.parse(text);
            jsonInput.value = JSON.stringify(parsed, null, 2);
          } catch {
            jsonInput.value = text; // keep raw content if not valid JSON
          }
      ...
      jsonContent.appendChild(loadJsonBtn);
      jsonContent.appendChild(fileInput);
      const convertBtn = document.createElement("button");
      convertBtn.textContent = "Convert to TOON";
      Utils.applyStyles(convertBtn, {
        marginTop: "8px",
        padding: "8px 16px",
        backgroundColor: "#5b3f87",
        color: "#fff",
      ...
      // Simple TOON encoder (basic but covers typical JSON)
      const jsonToToon = (obj, indent = 0) => {
        const spaces = "  ".repeat(indent);
        const nextSpaces = "  ".repeat(indent + 1);
        if (obj === null || obj === undefined) return "null";
        if (typeof obj === "boolean" || typeof obj === "number") return String(obj);
        if (typeof obj === "string") {
          // Escape special characters and quote if contains comma, newline, colon, or quote
          const escaped = obj.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
          if (/[\n,:"]/.test(escaped) || escaped === "") return `"${escaped}"`;
          return escaped;
        }
        if (Array.isArray(obj)) {
          if (obj.length === 0) return "[]";
          // Check if all items are objects with identical keys → tabular
          ...
          if (allObjects && obj.length > 0) {
            ...
            if (uform) {
              const keysStr = firstKeys.join(",");
              const header = `[${obj.length}]{${keysStr}}`;
              const rows = obj.map((item) =>
                firstKeys.map((k) => {
                  const val = item[k];
                  return val === null || val === undefined ? "" : jsonToToon(val, 0); // inline encoding
                }).join(",")
              );
              return rows.map((row) => `${nextSpaces}${row}`).join("\n");
            }
          }
          // Fallback: YAML-like list
          return obj
        ...
        if (typeof obj === "object") {
          ...
          return entries
            .map(([key, value]) => {
              const valStr = jsonToToon(value, indent + 1);
      ...
      jsonContent.appendChild(convertBtn);
      jsonContent.appendChild(jsonOutput);
      jsonContent.appendChild(copyJsonBtn);

      // Tab switching logic
      textTab.onclick = () => {
        textTab.style.borderBottomColor = "#5b3f87";
        textTab.style.color = "#5b3f87";
        jsonTab.style.borderBottomColor = "transparent";
        jsonTab.style.color = "#555";
        textContent.style.display = "block";
        jsonContent.style.display = "none";
      };
      jsonTab.onclick = () => {
        jsonTab.style.borderBottomColor = "#5b3f87";
        jsonTab.style.color = "#5b3f87";
        textTab.style.borderBottomColor = "transparent";
        textTab.style.color = "#555";
      ...
      // Overlay must be inserted before modal in DOM
      document.body.appendChild(overlay);
      // Connect overlay removal to modal display changes
      const observer = new MutationObserver(() => {
        if (modal.style.display === "none") overlay.remove();
      });
    ...
    addExportControls() {
      ...
      if (CURRENT_PLATFORM === GEMINI) {
        ...
        scrollStatus.title = `Auto-scroll         isEnabled ? "ON" : "OFF"
      ...
      markdownButton.onclick = () => ChatExporter.itiateExport("markdown");
      ...
      contaer.appendChild(jsonButton);
    ...
    generateOutlineContent() {
      ...
      Utils.applyStyles(titleDiv, OUTLINE_T_PROPS);
    ...
    toggleOutlineCollapse() {
      ...
      if (UIManager._outlineIsCollapsed) {
        ...
        // Tgger a re-evaluation of search filter if it was active
     ...
     * This function uses an iterative approach to handle dynamic loing.
     */
